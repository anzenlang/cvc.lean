...
This would give us access to all the usual symbol goodies ({lean}`Sw.State.Idents`,
...
Dropping the heavy-handed _wouldn't-it-be-great_ bit, "state structures" also give access to
{lean}`Sw.State.PredicateAt` ({lean}`Sw.State.PredAt`) and {lean}`Sw.State.RelationAt`
({lean}`Sw.State.RelAt`). They let us express, respectively, state predicates at some unrolling
depth {lean}`Sw.State.PredicateAt k` and state relations between a state at `k` and a state at
`k + 1`: {lean}`Sw.State.RelationAt k`.
