...
%%%
authors := ["Adrien Champion <anzenlang.io>"]
%%%

Welcome to the [cvc.lean][cvc.lean] user manual.

Cvc.lean is a high-level API for the [cvc5 SMT solver][cvc5] written by [Adrien Champion][adrien]
([anzenlang][azn]) for the cvc5 team through [Cesare Tinelli][cesare]
...
The tools in this section come from the Verso namespace `Manual` in the module `Manual.Meta.Lean`.
